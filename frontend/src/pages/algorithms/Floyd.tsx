...
const Floyd: React.FC = () => {
  ...
  const algorithmData: AlgorithmData = {
    ...
    definition:
      "The Floyd-Warshall algorithm is a dynamic programming algorithm for finding shortest paths in a weighted graph with positive or negative edge weights (but no negative cycles). It computes the shortest path distances between every pair of vertices.",
    realWorldUse:
      "Used in dense graphs, networking (routing tables), transitive closure of a relation, inversion of real matrices, and finding the widest path problem.",
    pros: [
    ...
    workingSteps: [
      {
        ...
        description:
          "Initialize distance matrix D where D[i][j] is weight of edge (i,j) if it exists, 0 if i=j, and infinity otherwise.",
      ...
      },
      {
        ...
        title: "Triple Loop",
        description:
          "Iterate through all intermediate nodes k, and for each pair (i, j), check if path going through k is shorter.",
        codeSnippet:
          "for k in range(N):\n  for i in range(N):\n    for j in range(N):",
      },
      {
        ...
        title: "Relaxation",
        description:
          "Update distance: D[i][j] = min(D[i][j], D[i][k] + D[k][j])",
        codeSnippet:
    ...
    examples: [
      {
        ...
        explanation:
          "Iteratively updates distances using intermediate nodes 0, 1, 2, 3.",
      },
    ],
  };
