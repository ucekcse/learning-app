...
const DivideConquer: React.FC = () => {
  ...
  const findMinMaxViz = async () => {
    ...
    const solve = async (l: number, h: number): Promise<[number, number]> => {
      ...
      setMaxVal(finalMax);
      setMessage(
        `Combining: Left[${min1}, ${max1}] vs Right[${min2}, ${max2}] -> Min: ${finalMin}, Max: ${finalMax}`,
      );
      await sleep(1000);
    ...
    if (isPlayingRef.current) {
      setMessage(
        `Done! Global Minimum: ${finalMin}, Global Maximum: ${finalMax}`,
      );
      setMinVal(finalMin);
      setMaxVal(finalMax);
    }
  ...
  const algorithmData: AlgorithmData = {
    ...
    definition:
      "Divide and Conquer is a paradigm where the problem is divided into subproblems, solved recursively, and then combined. For finding the Maximum and Minimum in an array, this approach compares pairs of elements recursively, which is efficient for tournament-like comparisons.",
    realWorldUse:
      "Merge Sort, Quick Sort, Strassen's Matrix Multiplication, Fast Fourier Transform (FFT).",
    pros: [
    ...
    workingSteps: [
      {
        ...
        title: "Divide",
        description:
          "Split the array into two equal halves until the base case is reached (1 or 2 elements).",
      ...
      },
      {
        ...
        title: "Conquer",
        description:
          "Solve the problem for the two halves recursively. Find local min/max.",
        codeSnippet:
      ...
      },
      {
        ...
        title: "Combine",
        description:
          "Compare results from the two halves to get the global min and max.",
    ...
    <div className="min-h-screen bg-surface-50 dark:bg-dark-950 text-gray-900 dark:text-white p-3 sm:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="card">
          ...
          <span className="algorithm-badge bg-teal-600 text-white">
        ...
        <div className="card">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">
            Recursive Min/Max finding visualization.
          </p>

          <div className="flex gap-4 mb-4">
            <button
              onClick={() => resetVisualizer(inputs.array)}
              className="btn btn-secondary text-sm"
            >
              Reset
            </button>
            <button
              onClick={findMinMaxViz}
              disabled={isPlaying}
          ...
          <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded border border-zinc-200 dark:border-zinc-800 min-h-[300px]">
            <MinMaxVisualizer
              array={inputs.array}
              low={low}
              high={high}
          ...
          inputFields={[
            {
              ...
              type: "array",
              placeholder: "1000, 11, 445, 1, 330, 3000",
            },
          ]}
        />
