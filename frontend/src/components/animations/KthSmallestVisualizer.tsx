    ...
    <div className="space-y-6">
      <div className="bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 text-center">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Looking for {k}-th smallest element (Index {k - 1})
        </p>
      </div>
      ...
      <div className="flex justify-center items-end gap-1 sm:gap-2 h-48 sm:h-64 bg-zinc-100 dark:bg-zinc-900/50 p-2 sm:p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-x-auto scrollbar-thin">
        {array.map((value, idx) => {
          ...
          if (foundIndex === idx) {
            ...
            // Inactive
            bgColor = "bg-zinc-400 dark:bg-zinc-700";
          ...
          return (
            ...
            >
              ...
              <div className="absolute -bottom-8 flex flex-col items-center">
                {idx === left && (
                  <span className="text-xs text-cyan-400 font-bold">L</span>
                )}
                {idx === right && (
                  <span className="text-xs text-cyan-400 font-bold">R</span>
                )}
                {idx === pivotIndex && (
                  <span className="text-xs text-yellow-400 font-bold">P</span>
                )}
              </div>
            </div>
          );
