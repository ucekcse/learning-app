...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  const executeCode = async () => {
    setIsRunning(true);
    setOutput(
      activeLanguage === "c" ? "Running simulation..." : "Running code...",
    );
    setHasError(false);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <textarea
            ...
            className={`w-full h-64 sm:h-96 bg-zinc-50/80 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg p-3 sm:p-4 font-mono text-xs sm:text-sm text-zinc-900 dark:text-gray-50 focus:outline-none focus:border-primary-500 resize-none scrollbar-thin placeholder-gray-400 dark:placeholder-gray-500 ${
              activeLanguage !== "python" ? "opacity-90" : ""
            }`}
            spellCheck={false}
          ...
          {activeLanguage !== "python" && (
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-2 text-right opacity-70">
              * C code is read-only. "Run Simulation" executes the equivalent
              Python logic.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <div
            className={`w-full h-64 sm:h-96 bg-zinc-50/80 dark:bg-zinc-950 border rounded-lg p-3 sm:p-4 font-mono text-xs sm:text-sm overflow-y-auto overflow-x-auto scrollbar-thin ${
              hasError
                ? "border-red-500 text-red-600 dark:text-red-400"
                : "border-zinc-300 dark:border-zinc-700 text-green-700 dark:text-green-400"
            }`}
          >
            {output ? (
              ...
              <span className="text-gray-400 dark:text-gray-500 italic">
                Hit "{activeLanguage === "c" ? "Run Simulation" : "Run Code"}"
                to see output...
              </span>
            )}
          </div>
